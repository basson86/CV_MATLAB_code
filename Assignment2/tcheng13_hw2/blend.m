...
%%
%Convert the image of the precision to double 
I1=double(I1);
I2=double(I2);
...
% Convert the 2 corner coordinates of I2 to I1 using H, to get the boundary
% of the transformed image
Corner1_m2t1= H*[n2;m2;1];
Corner1_m2t1=Corner1_m2t1/Corner1_m2t1(3);

Corner2_m2t1= H*[n2;0;1];
Corner2_m2t1=Corner2_m2t1/Corner2_m2t1(3);


mK21=Corner1_m2t1(2)-Corner2_m2t1(2);
nK21=max(Corner1_m2t1(1),Corner2_m2t1(1));

% determine the size of K by comparing the corner coordinate of I1 & I2 transformed to I1 
mK= max(ceil(mK21), ceil(m1));
nK= max(ceil(nK21), ceil(n1));

%% K1: the image of same size to K for storing the intensity of I1
K1=zeros(mK,nK);

% Yshift : the Y-coordinate shift of I1 due to the enlarged image frame
Yshift= ceil(-Corner2_m2t1(2));
...
I1_in=0;
 ...
 if (y_k>Yshift && y_k <=m1+Yshift && x_k <=n1)
 K1(y_k,x_k)= I1(y_k-Yshift,x_k);        
 I1_in=1;
 end

 %Transform the current coordinate (x_k,y_k) in K to I2 using 
 %"the inverse of H"
 X_2= inv(H)*[x_k;y_k-Yshift;1];
 X_2=X_2/X_2(3);
